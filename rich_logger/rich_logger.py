...
if __name__ == "__main__":
    ...
    sample_data = [4, 12, 8, 15, 3]
    accepted = []

    for i, value in enumerate(sample_data, start=1):
        logger.step(f"Record {i}: {value}")

        # Developer check: verify type and range
        logger.check(f"Check value type: {type(value).__name__}")
    ...
    logger.metric(
        f"accepted={len(accepted)} ignored={ignored} ratio={ratio:.2f}"
    )
